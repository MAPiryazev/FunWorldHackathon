...
func (s *Server) Start() {
	...
	// Восстанавливаем задачи из Redis при старте
	go func() {
		log.Println("[Server] Восстановление задач из Redis...")
		if err := s.notificationSvc.RecoverPendingTasks(ctx); err != nil {
			log.Printf("[Server] Ошибка восстановления задач: %v", err)
		}
	}()
