...
// Scan — сканирует ключи по паттерну (для внутреннего использования)
func (rc *RedisClient) Scan(ctx context.Context, cursor uint64, pattern string, count int) ([]string, uint64, error) {
	keys, newCursor, err := rc.client.Scan(ctx, cursor, pattern, int64(count)).Result()
	if err != nil {
		return nil, 0, fmt.Errorf("ошибка при сканировании Redis: %w", err)
	}
