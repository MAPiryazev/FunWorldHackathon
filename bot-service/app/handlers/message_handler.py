...
class MessageHandler:
    ...
    def __init__(self, nlp_client: NLPClient, state_manager: StateManager, reminder_handler: ReminderHandler):
        ...
        self.command_handler = CommandHandler(state_manager, self.reminder_client)
    ...
    async def handle_message(self, message: MaxMessage) -> BotResponse:
        ...
        if user_state == UserState.AWAITING_TIME:
            return await self._handle_time_response(user_id, chat_id, text)

        # 4. Обработка обычного текстового сообщения
        return await self._handle_text_message(user_id, chat_id, text)

    async def _handle_command(self, user_id: str, chat_id: str, text: str) -> BotResponse:
        ...
        try:
            ...
            if command in ["start", "начать"]:
                return await self.command_handler.handle_start(user_id, chat_id)

            elif command in ["help", "помощь"]:
                return await self.command_handler.handle_help(chat_id)

            elif command in ["cancel", "отмена"]:
                return await self.command_handler.handle_cancel(user_id, chat_id)

            elif command in ["tasks", "задачи"]:
                return await self.command_handler._handle_tasks(user_id, chat_id)
    ...
    async def _handle_text_message(self, user_id: str, chat_id: str, text: str) -> BotResponse:
        ...
        try:
            ...
            if nlp_result.get("remind_at"):
                ...
                return await self.reminder_handler.handle_reminder_creation(user_id, chat_id, nlp_result)
            else:
                # Время не указано - запрашиваем у пользователя
                self.logger.info("Время не указано, запрашиваем у пользователя")
                return await self.reminder_handler.handle_time_request(user_id, chat_id, text)
    ...
    async def _handle_time_response(self, user_id: str, chat_id: str, text: str) -> BotResponse:
